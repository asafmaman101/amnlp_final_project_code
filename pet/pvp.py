...
class PVP(ABC):
    ...
    def encode(self, example: InputExample, priming: bool = False, labeled: bool = False) \
        ...
        parts_a = [(tokenizer.encode(x, add_special_tokens=False, verbose=False, **kwargs), s) for x, s in parts_a if x]

        if parts_b:
            parts_b = [x if isinstance(x, tuple) else (x, False) for x in parts_b]
            parts_b = [(tokenizer.encode(x, add_special_tokens=False, verbose=False, **kwargs), s) for x, s in parts_b if x]
