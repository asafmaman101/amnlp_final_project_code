...
def run_experiment_given_arguments(args):

    timer = Timer('end-to-end')

    args.output_dir = args.output_dir.format(**args.__dict__)
    # args.output_dir += f'_{datetime.utcnow().strftime("%Y-%m-%d_%H-%M-%S")}'
    ...
    # TODO: change to support multi-task processing
...
def main():
    ...
    run_experiment_given_arguments(args)
